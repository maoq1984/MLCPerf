...
double  loss(LABEL y, LABEL ybar, int loss_function)
{
  ...
  else if(loss_function == PREC){
    loss=prec_loss(a,b,c,d);
  }
  else if(loss_function == REC){
    loss=rec_loss(a,b,c,d);
  }
  else if(loss_function == ACCURACY){
    loss=accuracy_loss(a,b,c,d);
  }
...
double eval_prediction(LABEL y, LABEL ypred,int loss_function)
{
	...
	else if(loss_function == PREC_K || loss_function == REC_K || loss_function == FONE || loss_function == PREC || loss_function == REC || loss_function == ACCURACY)
